  ...
  renderProductCard(item) {
    ...
    if (platform === 'TIKTOK_SHOP' || platform === 'RETAILER') {
      if (linkStatus === 'ACTIVE' && primaryLink) {
        ctaHtml = `<a href="${primaryLink}" target="_blank" rel="noopener noreferrer" class="btn btn-accent btn-sm">🛒 Đặt mua (${platform.replace('_', ' ')})</a>`;
      } else {
        ctaHtml = `<button class="btn btn-secondary btn-sm disabled" disabled>Link cần cập nhật</button>`;
      }
    } else {
      if (primaryLink) {
        ctaHtml = `<a href="${primaryLink}" target="_blank" rel="noopener noreferrer" class="btn btn-outline btn-sm">🌐 Xem thông tin hãng</a>`;
      } else {
